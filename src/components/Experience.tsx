  ...
  return (
    <section id="experience" className="py-24 border-t border-border/40 relative">
      <div className="container max-w-5xl mx-auto px-4 md:px-6">
        ...
        {/* Timeline Container */}
        <div className="relative pl-6 md:pl-8 border-l border-border/60 space-y-12">
          {experience.map((item) => (
            <div key={item.id} className="relative group">
              {/* Timeline Node Icon */}
              <div className="absolute -left-[31px] md:-left-[39px] top-1.5 w-6 h-6 rounded-full bg-background border-2 border-primary flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                <div className="w-2 h-2 rounded-full bg-primary" />
              </div>

              {/* Card Container */}
              <Card className="bg-card/40 backdrop-blur-sm border-border/60 hover:border-primary/40 transition-all duration-300">
